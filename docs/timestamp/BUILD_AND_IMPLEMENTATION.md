...
> 用途：把归档调研 [archive/timestamp_analysis.md](archive/timestamp_analysis.md) 的设计落地成当前项目代码。
> 编写：Clara，2026-05-20。
...
- `reference_submission/retrieval/timeparse.py` 提供 `TimeIndex` 查询 API。
- `reference_submission/build_time_index.py` 负责构建 `dataset/time_index.json`。
- `reference_submission/agents/review.py` 在初始化时加载 `TimeIndex`，备案日和财季末 exact 校验会共享该索引。
- `dataset/time_index.json` 已在本地构建，当前规模为 50 tickers、84331 accessions、2191 fiscal periods、172 earnings periods、250 trading days。

项目级使用说明见 [README.md](README.md)，原始交接适配步骤见 [archive/handoff_integration.md](archive/handoff_integration.md)。
...
| `dataset/time_index.json` | **生成产物**：跨源规范时间锚索引 | 当前约 22 MB |
...
这些走新闻覆盖路线，当前相关文档见 [../news-coverage.md](../news-coverage.md)。
